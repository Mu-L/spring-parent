...
import com.emily.infrastructure.logback.configuration.context.DefaultLoggerBeanFactory;
...
public class ContextServiceProvider implements ContextProvider {
    ...
    public void initialize(LoggerContext lc, LogbackProperties properties) {
        ...
        // 注册日志对象
        DefaultLoggerBeanFactory.registerBean(lc, properties);
        ...
        properties.getMarker().getAcceptMarker().forEach((marker) -> {
            lc.addTurboFilter(DefaultLoggerBeanFactory.getFilter().getAcceptMarkerFilter(marker));
        });
        //全局过滤器，拒绝标记的日志记录到文件中
        properties.getMarker().getDenyMarker().forEach((marker) -> {
            lc.addTurboFilter(DefaultLoggerBeanFactory.getFilter().getDenyMarkerFilter(marker));
        });
        start();
    }
    ...
    public <T> Logger getLogger(Class<T> clazz, String filePath, String fileName, LogbackType logbackType) {
        ...
        if (logger == null) {
            synchronized (ContextServiceProvider.class) {
                if (logger == null) {
                    // 获取logger日志对象
                    logger = DefaultLoggerBeanFactory.getLogger(commonKeys);
                    // 存入缓存
    ...
    public void start() {
        // 获取root logger对象
        Logger rootLogger = DefaultLoggerBeanFactory.getLogger(CommonKeys.newBuilder()
                // logger name
