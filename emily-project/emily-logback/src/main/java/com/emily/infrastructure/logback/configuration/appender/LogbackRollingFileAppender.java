...
public class LogbackRollingFileAppender extends AbstractAppender {
    ...
    protected Appender<ILoggingEvent> getAppender(Level level) {
        ...
        //设置日志文件归档策略
        appender.setRollingPolicy(DefaultLoggerBeanFactory.getRollingPolicy(appender, loggerPath, rp));
        ...
        //设置过滤器
        appender.addFilter(DefaultLoggerBeanFactory.getFilter().getLevelFilter(level));
        //设置附加器编码
        appender.setEncoder(DefaultLoggerBeanFactory.getEncoder(this.resolveFilePattern()));
